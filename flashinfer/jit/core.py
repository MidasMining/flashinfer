...
def check_cuda_arch():
    ...
    for major, minor in current_compilation_context.TARGET_CUDA_ARCHS:
        if major >= 8:
            ...
            # SM70 (Volta) has limited support: no ldmatrix, no bf16,
            # uses m8n8k4 MMA instead of m16n8k8/k16.
            # SM75 (Turing) and above have full support.
            eligible = True

    # Raise error only if all detected architectures are lower than sm70
    if not eligible:
        raise RuntimeError("FlashInfer requires GPUs with sm70 or higher")
...
def gen_jit_spec(
    ...
    # BF16 requires SM80+; exclude for SM70-only builds to avoid
    # nv_bfloat16 intrinsic compilation errors on Volta
    if any(
        major >= 8 for major, _ in current_compilation_context.TARGET_CUDA_ARCHS
    ):
        cuda_cflags.append("-DFLASHINFER_ENABLE_BF16")
    if debug:
        cflags += ["-O0", "-g"]
        cuda_cflags += [
            "-g",
