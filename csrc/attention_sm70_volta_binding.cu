 ...
 * TVM-FFI bindings for SM70 (Volta/V100) FlashAttention.
 */
...
void flash_attention_sm70_forward(
    ...
    if (D == 64) {
        status = sm70::launch_flash_attention_sm70_forward<64>(
            q_ptr, k_ptr, v_ptr, out_ptr, lse_ptr,
            B, H, M, N, static_cast<float>(softmax_scale), is_causal, stream
        );
    } else if (D == 128) {
        status = sm70::launch_flash_attention_sm70_forward<128>(
            q_ptr, k_ptr, v_ptr, out_ptr, lse_ptr,
            B, H, M, N, static_cast<float>(softmax_scale), is_causal, stream
        );
    } else {
        TVM_FFI_ICHECK(false) << "Unsupported head dimension: " << D
                              << ". SM70 kernel supports D=64 or D=128.";
    }

    TVM_FFI_ICHECK(status == cudaSuccess)
        << "SM70 FlashAttention failed: " << cudaGetErrorString(status);
}

TVM_FFI_DLL_EXPORT_TYPED_FUNC(flash_attention_sm70_forward, flash_attention_sm70_forward);
